...
@interface Model() {
    ...
    //模型数据
    //以顶点（GLKVector3）+纹理（GLKVector2）+法线（GLKVectoe3）格式存放的数据，
    //纹理和法线可以不存在
...
//    [aCoder encodeBytes:&_center length:sizeof(Position)];
//    [aCoder encodeBytes:&_diameter length:sizeof(float)];
//    [aCoder encodeBytes:&_zDiameter length:sizeof(float)];
//    [aCoder encodeInteger:_numOfTriangles forKey:@"numOfTriangles"];
//    [aCoder encodeBytes:(void *)_pVertices length:sizeof(SceneVertex) * _numOfTriangles * 3];
//    [aCoder encodeBool:_ready forKey:@"ready"];
//    //分组
//    [aCoder encodeObject:_modelGroup forKey:@"modelGroup"];
//    [aCoder encodeObject:_materialDict forKey:@"materialDict"];
//    [aCoder encodeBool:_hasVertexCoord forKey:@"hasVertexCoord"];
//    [aCoder encodeBool:_hasNormal forKey:@"hasNormal"];
//    
//}
//- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
//    self = [super init];
//    if(self) {
//        [aDecoder decodeBytesWithReturnedLength:<#(nonnull NSUInteger *)#>]
////        _center.x = p->x;
////        _center.y = p->y;
////        _center.z = p->z;
//    }
...
-(instancetype) initWithPath:(NSString *) path delegate:(id<ModelDelegate>) delegate {
    ...
    if(self) {
        ...
        _verticesData = [NSMutableData data];
